...
# The following tidyverse packages are used as well: stringr,lubridate, tidyr
```
...
### Who has Sailor Shift Influenced? - Inward Directed Graph
...
#### Rendering the Inward network graph
```{r}
graph_inward <- visNetwork(nodes_inward_vis, edges_inward_final,
                           main = paste0(target_name," 'Influenced' Network Graph")) %>%
  visNodes(
    color = list(
...
### Sailor Shift "influenced" network - Inward directed Graph
...
### Sailor Shift "influenced by" network - Outward directed Graph
