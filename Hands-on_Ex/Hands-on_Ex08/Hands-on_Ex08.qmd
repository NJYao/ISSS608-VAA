...
```         
Reading layer `MP14_SUBZONE_WEB_PL' from data source    `C:\tskam\r4va\data\geospatial' using driver `ESRI Shapefile' Simple feature collection with 323 features and 15 fields Geometry type: MULTIPOLYGON Dimension:     XY Bounding box:  xmin: 2667.538 ymin: 15748.72 xmax: 56396.44 ymax: 50256.33 Projected CRS: SVY21
```
...
Notice that only the first ten records will be displayed. This is a default setting for brevity and to avoid overwhelming the console with excessively large tables. This behavior by using functions like `head()` or `tail()` to display a specific number of rows.
...
## **Multiple View with Facets (I have a twin brothers :))**
...
Using the steps learned in the previous lesson, install and load **sf**, **tmap** and **tidyverse** packages into R environment.
:::
